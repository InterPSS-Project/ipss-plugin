...
public class Ieeet1AndesSmibConformanceTest {
    ...
    private static final Path REFERENCE = Path.of("src", "test", "resources", "reference",
            "andes", "ieeet1-smib-line-trip.csv");
    ...
    void lineTripTrajectoryMatchesAndes() throws Exception {
        ...
        var context = new PSSEMultiFileLoader().loadDStab(
                ...
                CASE.resolve("SMIB_v33_genrou_ieeet1.dyr").toString());
        ...
        Ieee1968Type1Exciter exciter = (Ieee1968Type1Exciter) network
        ...
        assertTrue(algorithm.initialization(), "IEEET1 dynamic initialization");
        assertTrue(algorithm.performSimulation(), "IEEET1 line-trip simulation");
        ...
        System.out.printf(java.util.Locale.ROOT,
                "IEEET1 ANDES SMIB max errors: speed=%.9g@%.4f efd=%.9g@%.4f "
                        + "v1=%.9g@%.4f v2=%.9g@%.4f%n",
        ...
        System.out.printf(java.util.Locale.ROOT,
                "IEEET1 ANDES internal max errors: sensed=%.9g@%.4f regInput=%.9g@%.4f "
                        + "vr=%.9g@%.4f efdState=%.9g@%.4f se=%.9g@%.4f "
                        + "vfe=%.9g@%.4f feedback=%.9g@%.4f%n",
                ...
                internalMax[6], internalMaxTime[6]);
        for (int i = 0; i < internalMax.length; i++) {
            assertTrue(internalMax[i] < 7e-4, "internal-signal parity index " + i);
        }
    }
    ...
    private static List<double[]> readReference() throws Exception {
        ...
        for (String line : EmbeddedCsvTrajectoryValues.lines("ieeet1-smib-line-trip.csv")) {
    ...
    private static final class InternalStateMonitor extends StateMonitor {
        private final Ieee1968Type1Exciter exciter;
        private final List<double[]> internal = new ArrayList<>();

        private InternalStateMonitor(Ieee1968Type1Exciter exciter) { this.exciter = exciter; }
        ...
        public boolean onSimuEvent(DStabSimuEvent event) {
            ...
            if (accepted && event.getType() == DStabSimuEvent.PlotStepMachineStates
                ...
                internal.add(new double[] {exciter.getSensedVoltage(), exciter.getVoltageError(),
                        exciter.getRegulatorOutput(), exciter.getFieldVoltageState(),
                        exciter.getSaturationVoltage(), exciter.getCombinedFieldFeedback(),

...
public class HygovAndesSmibConformanceTest {
    ...
    private static final Path REFERENCE = Path.of("src", "test", "resources", "reference",
            "andes", "hygov-smib-line-trip.csv");

    @Test
    void lineTripTrajectoryMatchesAndesWhenTemporaryDroopScalingIsUnity() throws Exception {
        IpssCorePlugin.init();
        var context = new PSSEMultiFileLoader().loadDStab(
                CASE.resolve("SMIB_v33.raw").toString(),
                CASE.resolve("SMIB_v33_genrou_hygov.dyr").toString());
        ...
        PsseHygovGovernor governor = (PsseHygovGovernor) network
        ...
        assertTrue(algorithm.initialization(), "HYGOV dynamic initialization");
        assertTrue(algorithm.performSimulation(), "HYGOV line-trip simulation");
        ...
        System.out.printf(java.util.Locale.ROOT,
                "HYGOV ANDES SMIB max errors: speed=%.9g@%.4f pm=%.9g@%.4f "
                        + "v1=%.9g@%.4f v2=%.9g@%.4f%n",
                max[0], maxTime[0], max[1], maxTime[1], max[2], maxTime[2],
                max[3], maxTime[3]);
        System.out.printf(java.util.Locale.ROOT,
                "HYGOV ANDES internal max errors: wd=%.9g@%.4f pd=%.9g@%.4f "
                        + "filter=%.9g@%.4f temp=%.9g@%.4f dg=%.9g@%.4f "
                        + "gate=%.9g@%.4f flow=%.9g@%.4f head=%.9g@%.4f%n",
        ...
        assertTrue(max[1] < 1e-3, "mechanical-power parity");
        ...
        for (int i = 0; i < internalMax.length; i++) {
            assertTrue(internalMax[i] < 2e-3, "internal-signal parity index " + i);
        }
    }
    ...
    private static List<double[]> readReference() throws Exception {
        ...
        for (String line : EmbeddedCsvTrajectoryValues.lines("hygov-smib-line-trip.csv")) {
    ...
    private static final class InternalStateMonitor extends StateMonitor {
        private final PsseHygovGovernor governor;
        private final List<double[]> internal = new ArrayList<>();

        private InternalStateMonitor(PsseHygovGovernor governor) { this.governor = governor; }
        ...
        public boolean onSimuEvent(DStabSimuEvent event) {
            ...
            if (accepted && event.getType() == DStabSimuEvent.PlotStepMachineStates
                ...
                internal.add(new double[] {governor.getMachine().getSpeed() - 1.0,
                        governor.getGovernorInput(), governor.getGovernorFilterOutput(),
                        governor.getTemporaryDroopState(), governor.getDesiredGate(),
                        governor.getGatePosition(), governor.getWaterFlow(),
                        governor.getTurbineHead()});
            }
            return accepted;
        }

...
/** Full-solver GENSAL comparison against the checked-in Independent reference. */
public class GensalIndependentSmibConformanceTest {
    private static final double STEP = 1.0 / 240.0;
    private static final Path CASE = Path.of("testData", "adpter", "psse", "v33", "SMIB");

    @Test
    void threeCycleFaultMatchesIndependentBoundaryAndInternalStates() throws Exception {
        IpssCorePlugin.init();
        ...
        EmbeddedTrajectoryReference reference = EmbeddedTrajectoryReference.embedded("smib-gensal");
        ...
        double initialReferenceAngle = initial.value(angle1) - initial.value(angle2);

        double[] maximum = new double[9];
        double[] maximumTime = new double[9];
        for (var expected : reference.postEventSamples()) {
            // Independent emits an algebraic post-event sample at the exact event time,
            ...
            double[] referenceValues = {
                    ...
                    expected.value(angle1) - expected.value(angle2) - initialReferenceAngle,
        ...
        System.out.printf(Locale.ROOT,
                "GENSAL Independent max errors: v1=%.9g v2=%.9g pMW=%.9g qMvar=%.9g "
                + "angleDeg=%.9g speed=%.9g eqp=%.9g psiDp=%.9g psiQpp=%.9g%n",
                Arrays.stream(maximum).boxed().toArray());
        System.out.println("GENSAL Independent max-error times: " + Arrays.toString(maximumTime));

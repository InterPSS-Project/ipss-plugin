...
public class Esac5aAndesSmibConformanceTest {
    ...
    private static final Path REFERENCE=Path.of("src","test","resources","reference",
            "andes","esac5a-smib-line-trip.csv");
    ...
    @Test void lineTripTrajectoryMatchesNativeAndes()throws Exception{
        IpssCorePlugin.init();var context=new PSSEMultiFileLoader().loadDStab(
                ...
                CASE.resolve("SMIB_v33_genrou_esac5a.dyr").toString());
        ...
        algorithm.setTotalSimuTimeSec(5);algorithm.setOutPutPerSteps(1);
        ...
        Esac5aExciter exciter=(Esac5aExciter)network.getMachine("Bus1-mach1").getExciter();
        ...
        assertTrue(algorithm.initialization(),"ESAC5A dynamic initialization");
        assertTrue(algorithm.performSimulation(),"ESAC5A line-trip simulation");
        ...
        Series[] internal=monitor.internalSeries(speed.time);double[] max=new double[12];
        double[] maxTime=new double[12];
        for(double[] row:readReference()){
            double time=row[0];double[] actual={speed.at(time),efd.at(time),bus1.at(time),bus2.at(time),
                    internal[0].at(time),internal[1].at(time),internal[2].at(time),internal[3].at(time),
                    internal[4].at(time),internal[5].at(time),internal[6].at(time),internal[7].at(time)};
        ...
        System.out.printf(java.util.Locale.ROOT,
                "ESAC5A ANDES SMIB max errors: speed=%.9g@%.4f efd=%.9g@%.4f "
        ...
        assertTrue(max[1]<5e-4,"field-voltage parity: "+max[1]);
        assertTrue(max[2]<5e-5,"generator-bus voltage parity: "+max[2]);
        assertTrue(max[3]<2e-5,"infinite-bus voltage parity: "+max[3]);
        for(int i=4;i<max.length;i++)assertTrue(max[i]<8e-4,
                "ESAC5A internal-state parity index "+(i-4)+": "+max[i]+" at "+maxTime[i]);
    }
    ...
    private static List<double[]> readReference()throws Exception{
        List<double[]> rows=new ArrayList<>();for(String line:EmbeddedCsvTrajectoryValues.lines("esac5a-smib-line-trip.csv")){
    ...
    private static final class InternalStateMonitor extends StateMonitor{
        private final Esac5aExciter exciter;private final List<double[]> internal=new ArrayList<>();
        private InternalStateMonitor(Esac5aExciter exciter){this.exciter=exciter;}
        @Override public boolean onSimuEvent(DStabSimuEvent event){boolean accepted=super.onSimuEvent(event);
            if(accepted&&event.getType()==DStabSimuEvent.PlotStepMachineStates
                ...
                internal.add(new double[]{exciter.getSensedVoltage(),exciter.getRegulatorState(),
                        exciter.getFeedback1State(),exciter.getFeedback2State(),
                        exciter.getInternalFieldVoltage(),exciter.getFeedback1Output(),
                        exciter.getRateFeedback(),exciter.getFieldFeedback()});}
            return accepted;}
        private Series[] internalSeries(double[] time){Series[] result=new Series[8];

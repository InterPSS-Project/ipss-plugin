...
/** Full-stack trajectory comparison against native ANDES 2.0.0 EXAC1. */
public class Exac1AndesSmibConformanceTest {
    ...
    @Test void lineTripTrajectoryMatchesNativeAndes() throws Exception {
        ...
        Exac1Exciter exciter=(Exac1Exciter)network.getMachine("Bus1-mach1").getExciter();
        ...
        assertTrue(algorithm.initialization(),"EXAC1 dynamic initialization");
        assertTrue(algorithm.performSimulation(),"EXAC1 line-trip simulation");
        ...
        Series[] internal=monitor.internalSeries(speed.time);double[] max=new double[9];
        double[] maxTime=new double[9];
        ...
        System.out.printf(java.util.Locale.ROOT,
                "EXAC1 ANDES SMIB max errors: speed=%.9g@%.4f efd=%.9g@%.4f "
        ...
        for(int i=4;i<max.length;i++)assertTrue(max[i]<internalTolerance[i-4],
                "EXAC1 internal-state parity index "+(i-4)+": "+max[i]+" at "+maxTime[i]);
    }
    ...
    private static List<double[]> readReference() throws Exception {
        List<double[]> rows=new ArrayList<>();for(String line:EmbeddedCsvTrajectoryValues.lines("exac1-smib-line-trip.csv")){
    ...
    private static final class InternalStateMonitor extends StateMonitor {
        private final Exac1Exciter exciter;private final List<double[]> internal=new ArrayList<>();
        private InternalStateMonitor(Exac1Exciter exciter){this.exciter=exciter;}

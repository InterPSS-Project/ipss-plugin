...
/** Trajectory comparison against the common implemented subset of ANDES 2.0.0 ESST1A. */
public class Esst1aAndesSmibConformanceTest {
    ...
    private static final Path REFERENCE = Path.of("src", "test", "resources", "reference",
            "andes", "esst1a-smib-line-trip.csv");
    ...
    void lineTripTrajectoryMatchesAndes() throws Exception {
        ...
        IEEE1981ST1Exciter exciter = (IEEE1981ST1Exciter) network
        ...
        assertTrue(algorithm.initialization(), "ESST1A dynamic initialization");
        assertTrue(algorithm.performSimulation(), "ESST1A line-trip simulation");
        ...
        System.out.printf(java.util.Locale.ROOT,
                "ESST1A ANDES SMIB max errors: speed=%.9g@%.4f efd=%.9g@%.4f "
                        + "v1=%.9g@%.4f v2=%.9g@%.4f%n",
                max[0], maxTime[0], max[1], maxTime[1], max[2], maxTime[2],
                max[3], maxTime[3]);
        assertTrue(max[0] < 2e-5, "rotor-speed parity");
        assertTrue(max[1] < 2e-4, "field-voltage parity");
        ...
        for (double[] row : readReference()) {
            double time = row[0];
            if (Math.abs(time - 1.0) <= STEP / 2.0) continue;
        ...
        System.out.printf(java.util.Locale.ROOT,
                "ESST1A ANDES internal max errors: sensed=%.9g@%.4f vi=%.9g@%.4f "
                        + "limitedVi=%.9g@%.4f ll1=%.9g@%.4f ll2=%.9g@%.4f "
                        + "va=%.9g@%.4f postLR=%.9g@%.4f preEfd=%.9g@%.4f "
                        + "feedback=%.9g@%.4f%n",
                ...
                internalMax[8], internalMaxTime[8]);
        for (int i = 0; i < internalMax.length; i++) {
            assertTrue(internalMax[i] < 2e-4, "internal-signal parity index " + i);
        }
    }
    ...
    private static List<double[]> readReference() throws Exception {
        ...
        for (String line : EmbeddedCsvTrajectoryValues.lines("esst1a-smib-line-trip.csv")) {
    ...
    private static final class InternalStateMonitor extends StateMonitor {
        private final IEEE1981ST1Exciter exciter;
        private final List<double[]> internal = new ArrayList<>();

        private InternalStateMonitor(IEEE1981ST1Exciter exciter) {
        ...
        public boolean onSimuEvent(DStabSimuEvent event) {
            ...
            if (accepted && event.getType() == DStabSimuEvent.PlotStepMachineStates
                ...
                internal.add(new double[] {exciter.getSensedVoltage(),
                        exciter.getVoltageErrorInput(), exciter.getLimitedVoltageError(),
                        exciter.getLeadLagOutput(), exciter.getLeadLag1Output(),
                        exciter.getRegulatorOutput(), exciter.getPostFieldCurrentLimiterSignal(),
                        exciter.getPreFieldVoltageSignal(), exciter.getRateFeedback()});
            }
            return accepted;
        }

...
public class DStab_IEEE9Bus_Test extends DStabTestSetupBase{
	...
	//@Test
	public void IEEE9_Dstab_multiGen_Test2(){
		IpssCorePlugin.init();
		IpssLogger.getLogger().setLevel(Level.INFO);
		PSSEAdapter adapter = new PSSEAdapter(PsseVersion.PSSE_30);
		assertTrue(adapter.parseInputFile(NetType.DStabNet, new String[]{
		...
		list = ssRecorder.getMachineRecords(
				"Bus3-mach1", StateVarRecType.MachineState, DStabOutSymbol.OUT_SYMBOL_MACH_ANG);
		System.out.println("\n\n Bus3 Machine-1 ANG");
		for (StateRecord rec : list) {
			System.out.println(Number2String.toStr(rec.t) + ", " + Number2String.toStr(rec.variableValue));
		}
		list = ssRecorder.getMachineRecords(
				"Bus3-mach1", StateVarRecType.MachineState, DStabOutSymbol.OUT_SYMBOL_MACH_PE);
		System.out.println("\n\n Bus3 Machine-1 PE");
		for (StateRecord rec : list) {
			System.out.println(Number2String.toStr(rec.t) + ", " + Number2String.toStr(rec.variableValue));
		}

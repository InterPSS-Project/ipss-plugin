...
package org.interpss.fadapter.epc;
...
import org.interpss.fadapter.psse.PSSEGeneratorReactivePower;
...
public class EpcDirectParser {
    ...
    private final Set<String> parsedSvdSignatures = new HashSet<>();
    private final Set<String> voltageControlSvdBusIds = new HashSet<>();
    private final Map<String, Double> busVoltageSetpoints = new HashMap<>();
    private double baseMva = 100.0;

    public EpcDirectParser() {
    ...
    public AclfNetwork parse(String filepath) throws InterpssException {
        try (BufferedReader reader = new BufferedReader(new FileReader(filepath))) {
            ...
            throw new InterpssException("Error reading EPC file: " + filepath + ": " + e.getMessage());
        }
        return builder.getNetwork();
    }

    private void preScanSvdBuses(List<String> lines) {
        voltageControlSvdBusIds.clear();
        String section = null;
        for (String line : lines) {
            String trimmed = line.trim();
            if (trimmed.isEmpty()) continue;
            String upper = trimmed.toUpperCase();
            if (upper.startsWith("SVD DATA")) {
                section = "SVD";
                continue;
            }
            ...
            if (!"SVD".equals(section) || trimmed.startsWith("//") || trimmed.startsWith("#")
                    || trimmed.startsWith("!")) {
                continue;
            }
            int colonIdx = trimmed.indexOf(':');
            if (colonIdx < 0) continue;
            String[] before = tokenizeQuoted(trimmed.substring(0, colonIdx));
            String[] after = tokenizeQuoted(trimmed.substring(colonIdx + 1));
            if (before.length > 0 && after.length > 1) {
                int busNum = intVal(before, 0);
                int mode = intVal(after, 1);
                if (busNum > 0 && (mode == 1 || mode == 2)) {
                    voltageControlSvdBusIds.add(BUS_ID_PREFIX + busNum);
                }
            }
        }
    }

    private static boolean isSectionHeader(String upper) {
        return upper.startsWith("SOLUTION PARAMETERS") ||
                upper.startsWith("TITLE") ||
                upper.startsWith("COMMENTS") ||
                upper.startsWith("BUS DATA") ||
                upper.startsWith("GENERATOR DATA") ||
                upper.startsWith("LOAD DATA") ||
                upper.startsWith("BRANCH DATA") ||
                upper.startsWith("LINE DATA") ||
                upper.startsWith("TRANSFORMER DATA") ||
                upper.startsWith("SHUNT DATA") ||
                upper.startsWith("AREA DATA") ||
                upper.startsWith("ZONE DATA") ||
                upper.startsWith("SVD DATA") ||
                upper.startsWith("Z TABLE DATA") ||
                upper.startsWith("INTERFACE DATA") ||
                upper.startsWith("DC BUS") ||
                upper.startsWith("DC LINE") ||
                upper.startsWith("DC CONVERTER") ||
                upper.startsWith("QTABLE DATA") ||
                upper.startsWith("GCD DATA") ||
                upper.startsWith("TRANSACTION") ||
                upper.startsWith("OWNER DATA") ||
                upper.startsWith("MOTOR DATA") ||
    ...
    public AclfNetwork parseFromReader(BufferedReader reader) throws IOException, InterpssException {
        ...
        preScanSvdBuses(lines);
        return parseLines(new BufferedReader(new StringReader(String.join(System.lineSeparator(), lines))));
    }

    private AclfNetwork parseLines(BufferedReader reader) throws IOException, InterpssException {
        String line;
        String section = null;
        ...
        while ((line = reader.readLine()) != null) {
            ...
            if (upper.startsWith("SVD DATA")) { section = "SVD"; continue; }
            if (upper.startsWith("Z TABLE DATA")) { section = "ZTABLE"; continue; }
            if (upper.startsWith("INTERFACE DATA") ||
                upper.startsWith("DC BUS") || upper.startsWith("DC LINE") ||
                upper.startsWith("DC CONVERTER") || upper.startsWith("QTABLE DATA") ||
            ...
            switch (section) {
                ...
                case "SVD":    parseSvdLine(trimmed, reader); break;
                case "AREA":   parseAreaLine(trimmed); break;
                case "ZONE":   parseZoneLine(trimmed); break;
                case "ZTABLE": parseZTableLine(trimmed); break;
            }
        }

        PSSEGeneratorReactivePower.finalizeBusTypes(builder);
        builder.setNetworkInfo("EPC_Case", "EPC Case", baseMva * 1000.0, OriginalDataFormat.GE_PSLF);
    ...
    private void parseBusLine(String line) throws InterpssException {
        ...
        if (vsched > 0.0) {
            busVoltageSetpoints.put(busId, vsched);
        }
    ...
    private void parseGenLine(String line, BufferedReader reader) throws IOException, InterpssException {
        ...
        int regBusNum = intVal(after, 1);
        String remoteVControlBusId = regBusNum > 0 && regBusNum != busNum ? BUS_ID_PREFIX + regBusNum : null;
        // positions 4-5: prf qrf
        double prf = dblVal(after, 4);
        ...
        qgen = snapToLimit(qgen, qmax, qmin, 1.0e-4);
        ...
        double busVs = 1.0;
        if (bus != null && bus.getVoltageMag() > 0) {
            busVs = bus.getVoltageMag();
        }
        String controlledBusId = remoteVControlBusId != null ? remoteVControlBusId : busId;
        Double controlledBusScheduledVoltage = busVoltageSetpoints.get(controlledBusId);
        Double localScheduledVoltage = busVoltageSetpoints.get(busId);
        double controlBusVs = busVs;
        if (remoteVControlBusId == null && voltageControlSvdBusIds.contains(busId) && prf > 0.0) {
            controlBusVs = prf;
        } else if (controlledBusScheduledVoltage != null && controlledBusScheduledVoltage > 0.0) {
            controlBusVs = controlledBusScheduledVoltage;
        } else if (localScheduledVoltage != null && localScheduledVoltage > 0.0) {
            controlBusVs = localScheduledVoltage;
        }
        builder.addContributeGen(busId, genId, status == 1,
                pgen / baseMva, qgen / baseMva, mbase, controlBusVs,
                qmax / baseMva, qmin / baseMva, pmax / baseMva, pmin / baseMva,
                null, null, 1.0, remoteVControlBusId, 1.0, 1.0);

        // Set PV bus parameters if bus is a PV generator bus
        if (bus != null && bus.getGenCode() == AclfGenCode.GEN_PV
                && status == 1 && qmax > qmin) {
            builder.setPVBus(busId, pgen / baseMva, controlBusVs,
    ...
    private static double snapToLimit(double value, double max, double min, double tolerance) {
    ...
    private void parseXfrLine(String line, BufferedReader reader) throws IOException, InterpssException {
        ...
        if (line4 != null && !isContinuationLine(line4)) {
            reader.reset();
        }
        ...
        if (parseExplicitStarXfr(fromNum, fromKV, toNum, toKV, ckt, status, afterTokens, line2)) {
            return;
        }

        int zTableNumber = afterTokens.length > 5 ? intVal(afterTokens, 5) : 0;
        ...
        if (line2 != null) {
            ...
            if (t2.length >= 2) {
                ...
                angle = t2.length > 3 ? dblVal(t2, 3) : 0.0;
                if (angle == 0.0 && t2.length > 2) {
                    angle = dblVal(t2, 2);
                }
                rate1 = t2.length > 6 ? dblVal(t2, 6) : 0.0;
                psAngMax = t2.length > 11 ? dblVal(t2, 11) : 0.0;
                psAngMin = t2.length > 12 ? dblVal(t2, 12) : 0.0;
                psPMax = t2.length > 13 ? dblVal(t2, 13) : 0.0;
                psPMin = t2.length > 14 ? dblVal(t2, 14) : 0.0;
                psControlCode = t2.length > 15 ? intVal(t2, 15) : 0;

                if (tap2KV > 0 && toKV > 0 && Math.abs(tap2KV - toKV) > 1.0e-6) {
                    double baseRatio = tap2KV / toKV;
                    r *= baseRatio * baseRatio;
                    x *= baseRatio * baseRatio;
                }
                ...
                if (Math.abs(angle) > 0.0 && t2.length > 18) {
                    double tr1 = dblVal(t2, 17);
                    double tr2 = dblVal(t2, 18);
                    if (tr1 > 0) fromTap = tr1;
                    if (tr2 > 0) toTap = tr2;
                } else if (t2.length > 17) {
                    double tr1 = dblVal(t2, 16);
                    double tr2 = dblVal(t2, 17);
                    if (tr1 > 0) fromTap = tr1 * (tap1KV > 0 && fromKV > 0 ? tap1KV / fromKV : 1.0);
                    if (tr2 > 0) toTap = tr2 * (tap2KV > 0 && toKV > 0 ? tap2KV / toKV : 1.0);
                }
            }
        }
        ...
        if (angle != 0.0) {
            var branch = builder.addPsXformer(fromBusId, toBusId, ckt,
                    new Complex(r, x), fromTap, toTap, angle, 0.0,
                    null, null, rate1, 0, 0, zTableNumber, status == 1);
            if (psControlCode != 0) {
                builder.addPsXfrAngleRangeControl(branch.getId(), psControlCode > 0,
                        Math.max(psPMax, psPMin) / baseMva,
                        Math.min(psPMax, psPMin) / baseMva,
                        (psPMax + psPMin) * 0.5, UnitType.mW,
                        psAngMax, psAngMin,
                        true, true, false);
            }
        } else {
    ...
    private boolean parseExplicitStarXfr(int fromNum, double fromKV, int toNum, double toKV, String ckt, int status,
            String[] afterTokens, String line2) throws InterpssException {
        if (afterTokens.length < 21) {
            return false;
        }
        int neutralNum = intVal(afterTokens, 6);
        int tertNum = intVal(afterTokens, 9);
        if (neutralNum <= 0 || tertNum <= 0) {
            return false;
        }

        String neutralBusId = BUS_ID_PREFIX + neutralNum;
        String fromBusId = BUS_ID_PREFIX + fromNum;
        String toBusId = BUS_ID_PREFIX + toNum;
        String tertBusId = BUS_ID_PREFIX + tertNum;
        if (builder.getBus(neutralBusId) == null || builder.getBus(tertBusId) == null) {
            return false;
        }

        int zTableNumber = intVal(afterTokens, 5);
        double tbase = dblVal(afterTokens, 14);
        double baseRatio = tbase > 0 && tbase != baseMva ? baseMva / tbase : 1.0;
        Complex z12 = new Complex(dblVal(afterTokens, 15) * baseRatio, dblVal(afterTokens, 16) * baseRatio);
        Complex z23 = new Complex(dblVal(afterTokens, 17) * baseRatio, dblVal(afterTokens, 18) * baseRatio);
        Complex z31 = new Complex(dblVal(afterTokens, 19) * baseRatio, dblVal(afterTokens, 20) * baseRatio);
        Complex zFrom = z12.add(z23).subtract(z31).multiply(0.5);
        Complex zTo = z12.add(z31).subtract(z23).multiply(0.5);
        Complex zTert = z23.add(z31).subtract(z12).multiply(0.5);
        double tertKV = dblVal(afterTokens, 11);
        ...
        if (line2 != null) {
            ...
            String[] t2 = line2Str.split("\\s+");
            angle = t2.length > 3 ? dblVal(t2, 3) : 0.0;
            fromRate = t2.length > 6 ? dblVal(t2, 6) : 0.0;
            toRate = t2.length > 7 ? dblVal(t2, 7) : 0.0;
            tertRate = t2.length > 8 ? dblVal(t2, 8) : 0.0;
            psAngMax = t2.length > 11 ? dblVal(t2, 11) : 0.0;
            psAngMin = t2.length > 12 ? dblVal(t2, 12) : 0.0;
            psPMax = t2.length > 13 ? dblVal(t2, 13) : 0.0;
            psPMin = t2.length > 14 ? dblVal(t2, 14) : 0.0;
            ...
            double tap1KV = dblVal(t2, 0);
            double tap2KV = dblVal(t2, 1);
            double tap3KV = dblVal(t2, 2);
            fromTap = t2.length > 16 && dblVal(t2, 16) > 0.0
                    ? dblVal(t2, 16) * (tap1KV > 0 && fromKV > 0 ? tap1KV / fromKV : 1.0) : 1.0;
            toTap = t2.length > 17 && dblVal(t2, 17) > 0.0
                    ? dblVal(t2, 17) * (tap2KV > 0 && toKV > 0 ? tap2KV / toKV : 1.0) : 1.0;
            tertTap = t2.length > 18 && dblVal(t2, 18) > 0.0
                    ? dblVal(t2, 18) * (tap3KV > 0 && tertKV > 0 ? tap3KV / tertKV : 1.0) : 1.0;
        }

        if (angle != 0.0) {
            var branch = builder.addPsXformer(fromBusId, neutralBusId, ckt + "-1", zFrom, fromTap, 1.0, angle, 0.0,
                    null, null, fromRate, 0.0, 0.0, zTableNumber, status == 1);
            if (psControlCode != 0) {
                builder.addPsXfrAngleRangeControl(branch.getId(), psControlCode > 0,
                        Math.max(psPMax, psPMin) / baseMva,
                        Math.min(psPMax, psPMin) / baseMva,
            ...
            builder.addXformer2W(fromBusId, neutralBusId, ckt + "-1", zFrom, fromTap, 1.0,
                    null, null, fromRate, 0.0, 0.0, zTableNumber, status == 1);
        }
        builder.addXformer2W(neutralBusId, toBusId, ckt + "-2", zTo, 1.0, toTap,
                null, null, toRate, 0.0, 0.0, 0, status == 1);
        builder.addXformer2W(neutralBusId, tertBusId, ckt + "-3", zTert, 1.0, tertTap,
                null, null, tertRate, 0.0, 0.0, 0, status == 1);
        return true;
    }
    ...
    private void parseShuntLine(String line) throws InterpssException {
        ...
        int busNum = intVal(before, 0);
        int status = intVal(after, 0);
        // EPC shunt data fields after status: ar zone pu_mw pu_mvar
    ...
    // ==================== SVD / Switched Shunt Record ====================
    private void parseSvdLine(String line, BufferedReader reader) throws IOException, InterpssException {
        int colonIdx = line.indexOf(':');
        if (colonIdx < 0) return;
        String afterStr = line.substring(colonIdx + 1).trim();
        ...
        if (nextLine != null) {
            ...
            if (!nextTrimmed.isEmpty() && !nextTrimmed.startsWith("//") && !nextTrimmed.contains(":")) {
                line2 = nextLine;
            } else {
                reader.reset();
            }
        ...
        String[] before = tokenizeQuoted(line.substring(0, colonIdx));
        String[] after = tokenizeQuoted(afterStr);
        if (before.length < 1 || after.length < 9) return;

        int busNum = intVal(before, 0);
        int status = intVal(after, 0);
        int mode = intVal(after, 1);
        int remoteBusNum = intVal(after, 2);
        double binit = dblVal(after, 8);
        double vswLo = dblVal(after, 9);
        double vswHi = dblVal(after, 10);

        List<AclfNetworkBuilder.ShuntBlock> blocks = new ArrayList<>();
        if (line2 != null) {
            String[] blockTokens = tokenizeQuoted(line2);
            int pairs = Math.min(10, blockTokens.length / 2);
            for (int i = 0; i < pairs; i++) {
                int steps = intVal(blockTokens, i * 2);
                double bStep = dblVal(blockTokens, i * 2 + 1);
                if (steps > 0 || bStep != 0.0) {
                    blocks.add(new AclfNetworkBuilder.ShuntBlock(steps, bStep * baseMva, true));
                }
            }
        }
        ...
        if (vswHi <= 0.0 || vswLo <= 0.0 || vswHi < vswLo) {
            vswHi = 1.0;
            vswLo = 1.0;
        }
        String shuntId = before.length > 3 ? before[3] : "1";
        String signature = busId + "|" + shuntId + "|" + status + "|" + mode + "|" + binit + "|" + blocks;
        if (!parsedSvdSignatures.add(signature)) {
            return;
        }
        AclfAdjustControlMode controlMode = switch (mode) {
            case 1 -> AclfAdjustControlMode.DISCRETE;
            case 2 -> AclfAdjustControlMode.CONTINUOUS;
            default -> AclfAdjustControlMode.FIXED;
        };
        builder.addSwitchedShunt(busId, shuntId, status == 1,
                controlMode, AclfAdjustControlType.RANGE_CONTROL, binit, vswHi, vswLo, remoteBusId, blocks);
    }
    ...
    private void parseZTableLine(String line) {
        String[] tokens = tokenizeQuoted(line.trim());
        if (tokens.length < 5) return;

        int tableNum = intVal(tokens, 0);
        if (tableNum <= 0) return;

        List<XfrZCorrection> points = new ArrayList<>();
        double lastTap = Double.NEGATIVE_INFINITY;
        for (int i = 3; i + 1 < tokens.length; i += 2) {
            double tap = dblVal(tokens, i);
            double factor = dblVal(tokens, i + 1);
            if (!points.isEmpty() && tap <= lastTap) {
                break;
            }
            points.add(new XfrZCorrection(tap, factor));
            lastTap = tap;
        }
    ...
    private static int intVal(String[] t, int idx) {
        ...
        try { return Integer.parseInt(t[idx].trim()); }
            ...
            try { return (int) Math.rint(Double.parseDouble(t[idx].trim())); }

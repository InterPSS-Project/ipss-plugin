...
/** IEEE 421.5-2016 / PSS/E AC2C high-initial-response rotating AC exciter. */
@AnController(input="mach.vt",output="this.outputSignal",refPoint="this.reference",display={})
public final class Ac2cExciter extends AnnotateExciter implements IntegrationStepAware {
    ...
    public double tr,tb,tc,ka,ta,vamax,vamin,kb,efemax,efemin,te,vfemax,kh,kf,tf;
    public double kc,kd,ke,e1,se1,e2,se2,vemin,reference,outputSignal;

    public Ac2cExciter(String id,Ac2cData data,Machine machine) {
        super(id,"AC2C","PSS/E");this.data=data;this._data=data;setMachine(machine);
    }

    public Ac2cData getData(){return data;}
    ...
    @Override public boolean initStates(BaseDStabBus<?,?> bus,Machine machine) {
        ...
        if(!validLocation(oelLocation)||!validLocation(uelLocation)||!validLocation(sclLocation)
                ||tr<0||tb<0||ta<0||te<0||tf<0||kc<0||kd<0
                ||Math.abs(ka)<=EPS||Math.abs(kb)<=EPS)return false;
        ...
        state[LEAD_LAG]=error0;state[VA]=va0;state[VFE_LAG]=vfe0;
    ...
    private void loadAndCorrectParameters() {
        ...
        kb=data.getKb()==0.0?minimumResolvedTimeConstant():data.getKb();
        efemax=Math.max(data.getEfemax(),data.getEfemin());efemin=Math.min(data.getEfemax(),data.getEfemin());
        te=correctedMinimum(data.getTe());vfemax=data.getVfemax();kh=data.getKh();
        kf=data.getKf();tf=correctedMinimum(data.getTf());kc=data.getKc();kd=data.getKd();ke=data.getKe();
        e1=data.getE1();se1=data.getSe1();e2=data.getE2();se2=data.getSe2();vemin=data.getVemin();
    }
    ...
    private void derivatives(double[] x,double[] dx,Machine machine) {
        ...
        dx[VA]=vaRate;dx[VFE_LAG]=lagDerivative(a.vfe,x[VFE_LAG],tf);
    ...
    private Algebraic algebraicsForField(double[] x,Machine machine,double sensed,double field) {
        ...
        double fieldCommand=kb*(va-kh*vfe);
        double efe=clamp(takeoverLimiterOutput(fieldCommand),efemin,efemax);
        double limitedField=clamp(field,vemin,fieldUpperLimit(field,ifd));
        return new Algebraic(sensed,error,leadLag,va,fieldCommand,efe,limitedField,vfe,feedback);
    }

    private double solveAlgebraicField(double[] x,Machine machine,double sensed) {
        double value=Double.isFinite(x[VE])?x[VE]:vemin;
        for(int i=0;i<30;i++){
            double residual=algebraicResidual(value,x,machine,sensed);if(Math.abs(residual)<1e-11)break;
    ...
    public double getFieldCommand(){return algebraics(active,getMachine()).fieldCommand;}
    ...
    private record Algebraic(double sensed,double error,double leadLag,double va,double fieldCommand,double efe,double field,double vfe,double feedback){}

...
public class Exac2Exciter extends AnnotateExciter {
    private final Exac2Data data;
    public double one=1, tr, tb, tc, ka, ta, vamax, vamin, kb, vrmax, vrmin;
    public double integratorGain, kl, kh, kf, tf, kc, kd, ke, vlrEffective;
    ...
    @Override public boolean initStates(BaseDStabBus<?,?> bus,Machine machine) {
        ...
        if(data.getTe()<=0 || tf<=0 || kb<=0 || kl<=0) return false;
        integratorGain=1/data.getTe();

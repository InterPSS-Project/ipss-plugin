 ...
 */
public class PsseGastwddGovernor extends AbstractGovernor implements IntegrationStepAware, NamedDynamicStateProvider {
    ...
    @Override public boolean initStates(BaseDStabBus<?,?> bus, Machine mach) {
        ...
        double tempIntegral = maxLimit - effectiveT5/tauT*(d.getTc()-temperature);
    ...
    public double getPowerTransducer(){return state.powerDroop;} public double getFuelFlow(){return state.fuel;} public double getValvePosition(){return state.valve;} public double getRadiationShield(){return state.radiationLag;} public double getThermocouple(){return state.thermocouple;} public double getTurbineDynamics(){return state.gasLag;} public double getPidIntegralState(){return state.pidIntegral;} public double getSpeedCommand(){return algebraic(state,Math.max(dtSetting,EPS),time).speedCommand;} public double getTemperatureCommand(){return algebraic(state,Math.max(dtSetting,EPS),time).tempCommand;} public double getLowValueSelect(){return algebraic(state,Math.max(dtSetting,EPS),time).lowSelect;} public double getEffectiveMaxLimit(){return maxLimit;} public double getEffectiveMinLimit(){return minLimit;} public double applySpeedDeadband(double v){return deadband(v);}
    @Override public Map<String,Double> getNamedStates(){return Map.of("Power Transducer",getPowerTransducer(),"Valve Positioner",getValvePosition(),"Fuel System",getFuelFlow(),"Radiation Shield",getRadiationShield(),"Thermocouple",getThermocouple(),"Temp Control",getTemperatureCommand(),"Turbine Dynamics",getTurbineDynamics(),"PID 1",getPidIntegralState(),"PID 2",getSpeedCommand());}

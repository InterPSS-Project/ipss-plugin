package org.interpss.dstab.control.exc.psse.esurry;

/** PSS/E ESURRY (WECC EXAC1M) excitation-system parameters. */
public class EsurryData {
    private double tr, t1, ta, tb, tc, td, k10, k16, kf, tf;
    private double vrmax, vrmin, te, e1, se1, e2, se2, kc, kd, ke;

    public double getTr() { return tr; }
    public void setTr(double value) { tr = value; }
    public double getT1() { return t1; }
    public void setT1(double value) { t1 = value; }
    public double getTa() { return ta; }
    public void setTa(double value) { ta = value; }
    public double getTb() { return tb; }
    public void setTb(double value) { tb = value; }
    public double getTc() { return tc; }
    public void setTc(double value) { tc = value; }
    public double getTd() { return td; }
    public void setTd(double value) { td = value; }
    public double getK10() { return k10; }
    public void setK10(double value) { k10 = value; }
    public double getK16() { return k16; }
    public void setK16(double value) { k16 = value; }
    public double getKf() { return kf; }
    public void setKf(double value) { kf = value; }
    public double getTf() { return tf; }
    public void setTf(double value) { tf = value; }
    public double getVrmax() { return vrmax; }
    public void setVrmax(double value) { vrmax = value; }
    public double getVrmin() { return vrmin; }
    public void setVrmin(double value) { vrmin = value; }
    public double getTe() { return te; }
    public void setTe(double value) { te = value; }
    public double getE1() { return e1; }
    public void setE1(double value) { e1 = value; }
    public double getSe1() { return se1; }
    public void setSe1(double value) { se1 = value; }
    public double getE2() { return e2; }
    public void setE2(double value) { e2 = value; }
    public double getSe2() { return se2; }
    public void setSe2(double value) { se2 = value; }
    public double getKc() { return kc; }
    public void setKc(double value) { kc = value; }
    public double getKd() { return kd; }
    public void setKd(double value) { kd = value; }
    public double getKe() { return ke; }
    public void setKe(double value) { ke = value; }
}

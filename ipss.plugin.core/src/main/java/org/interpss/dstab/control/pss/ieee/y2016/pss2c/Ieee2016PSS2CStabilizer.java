...
/** IEEE Std 421.5-2016 dual-input PSS2C stabilizer. */
@AnController(input="mach.speed", output="this.outputBlock.y", refPoint="0.0", display={})
public class Ieee2016PSS2CStabilizer extends AnnotateStabilizer
    ...
    @AnControllerField(type=CMLFieldEnum.ControlBlock,
            input="this.refPoint + this.rampFilter.y - this.inputPath2.y",
    ...
    public double t12, t13, vstmax, vstmin;
    @AnControllerField(type=CMLFieldEnum.ControlBlock, input="this.leadLag3.y",
            parameter={"this.one", "this.t12", "this.t13", "this.vstmax", "this.vstmin"},
            y0="pss.vs")
    Pss2aLeadLagBlock outputBlock;

    public double tpgfilt;
    ...
    public Ieee2016PSS2CStabilizer() {
        this("id", "PSS2C", "IEEE-2016");
    }

    public Ieee2016PSS2CStabilizer(String id, String name, String category) {
        super(id, name, category);
        _data = new Ieee2016PSS2CStabilizerData();
    }

    public Ieee2016PSS2CStabilizerData getData() {
    ...
    public void configureIntegrationStep(double timeStepSec, double multiplier) {
        if (!Double.isFinite(timeStepSec) || timeStepSec < 0.0
            ...
            throw new IllegalArgumentException("PSS2C integration-step settings must be finite and non-negative");
        }
    ...
    public boolean initStates(BaseDStabBus<?, ?> bus, Machine machine) {
        Ieee2016PSS2CStabilizerData d = getData();
        // Capture the initialized electrical-power state before current access.
        // Some machine implementations refresh Pe while evaluating getIxy().
        ...
        t12 = d.getT12(); t13 = d.getT13();
        vsi1max = d.getVsi1max(); vsi1min = d.getVsi1min();
        vsi2max = d.getVsi2max(); vsi2min = d.getVsi2min();
        vstmax = d.getVstmax(); vstmin = d.getVstmin();
        tpgfilt = d.getTpgfilt(); m = d.getM(); n = d.getN();
        applyPowerWorldCorrections();

        input1PreviousVoltage = input1Bus.getVoltageMag();
        input2PreviousVoltage = input2Bus.getVoltageMag();
        previousCompensatedAngle = d.getIcs1() == 7
                ? compensatedVoltageAngle(machine, d.getXcomp()) : 0.0;
        compensatedFrequencyState = 0.0;
        input1Signal = limitedInput(d.getIcs1(), input1Bus, machine,
                vsi1max, vsi1min, 0.0, initialPgen);
        input2Signal = limitedInput(d.getIcs2(), input2Bus, machine,
                vsi2max, vsi2min, 0.0, initialPgen);
        ...
        pssActive = initialActivation(initialPgen, d);
        return initialized;
    }
    ...
    public boolean nextStep(double dt, DynamicSimuMethod method, Machine machine, int flag) {
        Ieee2016PSS2CStabilizerData d = getData();
        double instantaneousPgen = machine.getPe();
        double compensatedFrequency = d.getIcs1() == 7
                ? updateCompensatedFrequency(machine, d.getXcomp(), d.getTcomp(), dt, flag)
                : 0.0;
        input1Signal = d.getIcs1() == 6
                ? clamp((input1Bus.getVoltageMag() - input1PreviousVoltage) / dt,
                        vsi1max, vsi1min)
                : limitedInput(d.getIcs1(), input1Bus, machine,
                        vsi1max, vsi1min, compensatedFrequency, instantaneousPgen);
        input2Signal = d.getIcs2() == 6
                ? clamp((input2Bus.getVoltageMag() - input2PreviousVoltage) / dt,
                        vsi2max, vsi2min)
                : limitedInput(d.getIcs2(), input2Bus, machine,
                        vsi2max, vsi2min, 0.0, instantaneousPgen);
        double measuredPgen = updateFilteredPgen(instantaneousPgen, dt, flag);
        boolean result = super.nextStep(dt, method, machine, flag);
        if (flag != 0) {
            input1PreviousVoltage = input1Bus.getVoltageMag();
            input2PreviousVoltage = input2Bus.getVoltageMag();
            updateActivation(measuredPgen, d);
        }
        return result;
    }
    ...
    public double getFilteredPgen() {
        return filteredPgen;
    }

    private static boolean initialActivation(double pgen, Ieee2016PSS2CStabilizerData d) {
        return pgen >= d.getPssActivation();
    }

    private void updateActivation(double pgen, Ieee2016PSS2CStabilizerData d) {
        if (pssActive) {
            if (pgen <= d.getPssDeactivation()) pssActive = false;
        } else if (pgen >= d.getPssActivation()) {
            pssActive = true;
        }
    }

    private double updateFilteredPgen(double pgen, double dt, int flag) {
        if (tpgfilt <= 0.0 || dt <= 0.0) {
            filteredPgen = pgen;
            return filteredPgen;
        }
        if (flag == 0) {
            pgenFilterDerivative = (pgen - filteredPgen) / tpgfilt;
            pgenFilterTrial = filteredPgen + pgenFilterDerivative * dt;
            return pgenFilterTrial;
        }
        double correctedDerivative = (pgen - pgenFilterTrial) / tpgfilt;
        filteredPgen += 0.5 * (pgenFilterDerivative + correctedDerivative) * dt;
        return filteredPgen;
    }

    private double updateCompensatedFrequency(Machine machine, double xcomp,
            double tcomp, double dt, int flag) {
        if (dt <= 0.0) return compensatedFrequencyState;
        double angle = compensatedVoltageAngle(machine, xcomp);
        ...
        if (tcomp <= 0.0) {
            ...
            compensatedFrequencyDerivative = (raw - compensatedFrequencyState) / tcomp;
            compensatedFrequencyTrial = compensatedFrequencyState
                    + compensatedFrequencyDerivative * dt;
            return compensatedFrequencyTrial;
        } else {
            double correctedDerivative = (raw - compensatedFrequencyTrial) / tcomp;
    ...
    private static double compensatedVoltageAngle(Machine machine, double xcomp) {
        ...
        Complex compensatedVoltage = machine.getDStabBus().getVoltage()
                .add(terminalCurrentMachineBase.multiply(new Complex(0.0, xcomp)));
        return compensatedVoltage.getArgument();
    }
    ...
    private void applyPowerWorldCorrections() {
        ...
        t13 = quarterStepBypass(t13, minimum);
    ...
    private static double limitedInput(int code, BaseDStabBus<?, ?> bus, Machine machine,
        ...
        double value = switch (code) {
            ...
            default -> throw new IllegalArgumentException("Unsupported PSS2C input code: " + code);
        };

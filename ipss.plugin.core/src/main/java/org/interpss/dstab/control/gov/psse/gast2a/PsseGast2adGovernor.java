...
public class PsseGast2adGovernor extends AbstractGovernor implements IntegrationStepAware {
    ...
    @Override public boolean initStates(BaseDStabBus<?,?> bus,Machine mach){
        ...
        double tempIntegral=lvs-t5/tauT*(d.getTc()-temp);
    ...
    private Algebraic algebraic(State s,double at){var d=getData();double speed=deadband(getMachine().getSpeed()-1),u=pref-speed;double speedCommand;if(d.getZ()==0)speedCommand=s.speedState+d.getW()*d.getX()/y*u;else if(y>EPS)speedCommand=d.getW()*(d.getX()/y*u+(1-d.getX()/y)*s.speedState);else speedCommand=d.getW()*u;speedCommand=clamp(speedCommand,min,max);double combustor=delayed(combustorHistory,at-d.getEcr(),s.fuel),exhaust=delayed(exhaustHistory,at-etd,combustor);double f1=d.getTr()-d.getAf1()*(1-exhaust)-d.getBf1()*speed,shield=d.getK4()*f1+d.getK5()*s.radiationLag,tempError=d.getTc()-s.thermocouple,tempCommand=s.tempIntegral+t5/tauT*tempError,low=clamp(Math.min(speedCommand,tempCommand),min,max),delayedCommand=delayed(commandHistory,at-t,k3*low);return new Algebraic(speed,u,speedCommand,tempError,tempCommand,low,delayedCommand,combustor,f1,shield);}
    private Deriv derivatives(State s,Algebraic x){var d=getData();double gov;if(d.getZ()==0)gov=d.getW()/y*x.governorInput;else gov=y>EPS?(x.governorInput-s.speedState)/y:0;double valve=(a*(x.delayedCommand+d.getK6()-d.getKf()*s.fuel)-d.getC()*s.valve)/b,fuel=(s.valve-s.fuel)/tauF,radiation=(x.f1-s.radiationLag)/t3,thermo=(x.shield-s.thermocouple)/t4,tempInt=x.tempError/tauT,gas=tcd>EPS?(x.combustor-s.gasLag)/tcd:0;return new Deriv(gov,valve,fuel,radiation,thermo,tempInt,gas);}

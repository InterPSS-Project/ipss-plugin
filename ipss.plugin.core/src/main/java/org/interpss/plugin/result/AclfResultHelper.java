...
public class AclfResultHelper {
	...
	public AclfGenInfo getGenResult(AclfGen gen) {
		...
		genInfo.setGenType(    // PV, PQ, Swing, Load
				gen.getCode() == AclfGenCode.SWING? "Swing" :
					gen.getCode() == AclfGenCode.GEN_PV? "PV" :
						gen.getCode() == AclfGenCode.GEN_PQ? "PQ" : "NonGen"
				);  
		
		genInfo.setGen(gen.getGen().multiply(baseMva));
	...
	public AclfLoadInfo getLoadResult(AclfLoad load) {
		...
		loadInfo.setLoadType(    // ConstP, ConstI, ConstZ, ZIP
				load.getCode() == AclfLoadCode.CONST_P? "ConstP" :
					load.getCode() == AclfLoadCode.CONST_I? "ConstI" :
						load.getCode() == AclfLoadCode.CONST_Z? "ConstZ" : 
							load.getCode() == AclfLoadCode.ZIP? "ZIP" : "NonGen"
				);  
		
		loadInfo.setLoad(load.getLoad(bus.getVoltageMag()).multiply(baseMva));
		return loadInfo;
	}
	...
	public AclfBranchInfo getBranchResult(String branchId) {
		...
		branchInfo.setBranchPowerFlowSide2To(branch.powerFrom2To(UnitType.mVA));
		branchInfo.setBranchPowerFlowTo2From(branch.powerTo2From(UnitType.mVA));

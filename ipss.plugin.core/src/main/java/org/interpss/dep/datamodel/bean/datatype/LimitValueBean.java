...
public class LimitValueBean  implements Comparable<LimitValueBean> {
	private static final Logger log = LoggerFactory.getLogger(LimitValueBean.class);
	...
	@Override public int compareTo(LimitValueBean bean) {
		...
		if (!NumericUtil.equals(this.max, bean.max, CompareBaseJSONBean.PU_ERR)) {
			log.warn("BranchValueBean.f is not equal, " + this.max + ", " + bean.max); eql = 1; }
		
		if (!NumericUtil.equals(this.min, bean.min, CompareBaseJSONBean.PU_ERR)) {
			log.warn("BranchValueBean.t is not equal, " + this.min + ", " + bean.min); eql = 1; }	

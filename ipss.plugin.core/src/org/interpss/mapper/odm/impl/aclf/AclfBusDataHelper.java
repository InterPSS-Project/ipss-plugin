...
public class AclfBusDataHelper {
	...
	public void setAclfBusData(LoadflowBusXmlType xmlBusData) throws InterpssException {
		...
		//Set gen Code
		aclfBus.setGenCode(xmlBusData.getGenCode() == LFGenCodeEnumType.SWING? AclfGenCode.SWING : 
			xmlBusData.getGenCode() == LFGenCodeEnumType.PQ? AclfGenCode.GEN_PQ :
				xmlBusData.getGenCode() == LFGenCodeEnumType.PV? AclfGenCode.GEN_PV : AclfGenCode.NON_GEN);
		
		if (xmlBusData.getGenData()!=null && aclfBus.getGenCode()!=AclfGenCode.NON_GEN) {
			//mapGenData(xmlBusData.getGenData());
			mapContributeGenListData(xmlBusData.getGenData());
		...
		if (xmlBusData.getLoadData() != null) {
			...
			mapContributeLoadData(xmlBusData.getLoadData());
	...
	private void mapContributeGenListData(BusGenDataXmlType xmlGenData) throws InterpssException{
		...
		if(xmlGenData.getContributeGen()!=null){
			...
			for(JAXBElement<? extends LoadflowGenDataXmlType> elem :xmlGenData.getContributeGen()){
				...
				LFGenCodeEnumType genCode = xmlGen.getCode();
				
				if(xmlGen.getCode()!=LFGenCodeEnumType.NONE_GEN){
					...
					*/
					gen.setCode(aclfBus.getGenCode());
					if(xmlGen.getMvaBase()!=null)
					...
					if(xmlGen.getMvarVControlParticipateFactor()!=null)
...
//			mapContributeLoadData(xmlLoadData);
//		}
//	}

	private void mapContributeLoadData(BusLoadDataXmlType xmlLoadData){
			
		double baseKva = aclfBus.getNetwork().getBaseKva();
		if(xmlLoadData.getContributeLoad()!=null){
			if(xmlLoadData.getContributeLoad().size()>0){
				//TODO we set parent bus load code to constant power

...
@AnController(
		   input="mach.vt",
		   output="this.tmDelayBlock.y * this.vbGainBlock.y",
		   refPoint="this.viLimitBlock.u0 + this.washoutBlock.y - pss.vs  + this.trDelayBlock.y",
		   display= {})
public class IEEE2005ST3AExciter  extends AnnotateExciter{
	...
	//HV Gain
	   ...
	   @AnControllerField(
		   ...
		   y0="this.kgGainBlock.y + this.tmDelayBlock.u0"  )
	   DelayControlBlock taDelayBlock;
	
	
	//KG feedback, gain with upper limit
	     public double  vgmax = 8, vgmin = -9999;
		   @AnControllerField(
			   type= CMLFieldEnum.StaticBlock,
			   input="mach.efd",
			   parameter={"type.Limit", "this.k1", "this.vgmax", "this.vgmin"},
			   feedback = true)
	...
	//TM delayed block
	   ...
	   @AnControllerField(
		   ...
		   parameter={"type.NonWindup", "this.ka", "this.ta","this.vrmax","this.vrmin"},
		   y0="mach.efd/this.vbGainBlock.y"  )
	   DelayControlBlock tmDelayBlock;
	
	/*
	 * may consider calculate the ve inside the exciter, instead of using a function
	 */
	...
	//Multiply function: VB = VE*FEX
	   @AnFunctionField(
	      input= {"this.VeFunc.y","this.fexFunc.y"}
       )
	   MultiplyFunction multFunc;   

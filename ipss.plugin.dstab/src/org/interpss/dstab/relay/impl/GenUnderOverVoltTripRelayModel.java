...
public class GenUnderOverVoltTripRelayModel extends BusRelayModel {

	
	private String genId = "";
	private String voltBusId="";
	private BaseDStabBus voltMonitorBus = null;
	
	
	public GenUnderOverVoltTripRelayModel() {
		
	}
	
	public GenUnderOverVoltTripRelayModel(BaseDStabBus bus, String genId, String voltBusId){
		super(bus);
		this.setDStabBus(bus);
		this.genId = genId;
		
		if(voltBusId!=null) {
			voltMonitorBus = (BaseDStabBus) this.getDStabBus().getNetwork().getBus(voltBusId);
		}
		...
		this.extendedDeviceId = "GenUVTripRelay_"+bus.getId()+"_"+genId;
	...
	public boolean updateAttributes(boolean netChange) {
		...
		if(this.step_flag ==1) {
			   ...
			   if(this.voltMonitorBus!=null) 
				   volt = this.voltMonitorBus.getVoltageMag();
			   else
				   volt = this.getDStabBus().getVoltageMag();
					...
					for(int i = 0; i<this.relaySetPoints.size();i++){
						...
						if(!this.relayTrippedStatus.get(i)) {// only untripped or unactioned sections of the relay will be active
							...
							if(this.underOverFlagList.get(i)<=0) { // this setting point is for under-voltage
								if(this.relaySetPoints.get(i).getValue1()<volt){
									//reset timer
									timerAry[i] = 0.0;
								}
								else{
							...
							else if(this.underOverFlagList.get(i)>0) { // this setting point is for over-voltage
								if(this.relaySetPoints.get(i).getValue1()>volt){
									//reset timer
									timerAry[i] = 0.0;
								}
	...
	public void setVoltMonitorBusId(String voltMonBusId) {
		this.voltBusId = voltMonBusId;
	}

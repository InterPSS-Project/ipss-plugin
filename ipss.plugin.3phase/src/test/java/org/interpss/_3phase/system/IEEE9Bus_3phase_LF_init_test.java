	...
	public void test3PNetworkInitialization() throws InterpssException {
	  ...
	  for(BaseAcscBus bus: net.getBusList()){
		  if(bus instanceof DStab3PBus){
			  ...
			  if (ph3Bus.isGen()){
				 DStab3PGen gen = ph3Bus.getContributeGenList().get(0);
			  ...
			  if(bus.isLoad()){
				  DStab3PLoad ph3Load = ph3Bus.getContributeLoadList().get(0);
...
//			DStab3PGen gen1 = dsNet.getBus("Bus1").getContributeGenList().get(0);
			...
			gen2.getParentBus().initContributeGen(false);

			((BaseDStabBus)gen2.getParentBus()).resetSeqEquivLoad();
			...
			// applied the event
//			dsNet.addDynamicEvent(DStabObjectFactory.createBusFaultEvent("Bus9", dsNet, SimpleFaultCode.GROUND_3P,
//					new Complex(0.0), null, 0.1d, 0.05), "3phaseFault@Bus5");
//			 Generator trip event
			dsNet.addDynamicEvent(DStabObjectFactory.createGeneratorTripEvent("Bus3", "1", dsNet, 1),

...
public class OpenDSSQstsAdapterTest {
	@Test
	void factoryKeepsOpenDssMetadataAtAdapterBoundary() {
		...
		IPhaseGen generator = parser.getDistNetwork().getBus("bus1").getContributeGenList().get(0);
		Complex3x1 power = generator.getPower3Phase(UnitType.PU);
		assertEquals(0.75 * 60.0 / parser.getDistNetwork().getBaseKva() / 3.0,
				power.a_0.getReal(), 1.0e-12);
		assertEquals(0.0, power.a_0.getImaginary(), 1.0e-12);
		assertEquals(0.75 * 60.0 / parser.getDistNetwork().getBaseKva(),

...
def generator_power_rows(feeder: FeederCase, step: int, hour: float) -> Iterable[list[object]]:
    for generator_name in dss.Generators.AllNames() or []:
        dss.Generators.Name(generator_name)
        element_name = f"generator.{generator_name}".lower()
        dss.Circuit.SetActiveElement(element_name)
        powers = list(dss.CktElement.Powers())
        if not powers:
            continue
...
def export_case(
    ...
    try:
        ...
        if include_generator_powers:
            generator_handle = generator_file.open("w", newline="", encoding="utf-8")
            generator_writer = csv.writer(generator_handle)
            generator_writer.writerow([
                "case",
                "step",
                "hour",
                "device",
        ...
        for step in range(steps):
            ...
            if load_writer is not None:
                ...
                load_rows += len(rows)
            if generator_writer is not None:

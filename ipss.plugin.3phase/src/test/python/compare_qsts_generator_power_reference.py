#!/usr/bin/env python3
"""Compare DSS-Python and InterPSS QSTS generator-power exports."""
...
def read_generator_powers(path: Path, zero_threshold_kw: float) -> dict[tuple[int, str, str], GeneratorPower]:
    values: dict[tuple[int, str, str], GeneratorPower] = {}
    with path.open(newline="", encoding="utf-8") as handle:
        for row in csv.DictReader(handle):
            ...
            values[key] = GeneratorPower(p_kw, q_kvar)
    return values


def compare(
    dss: dict[tuple[int, str, str], GeneratorPower],
    interpss: dict[tuple[int, str, str], GeneratorPower],
...
def main() -> int:
    ...
    parser.add_argument("--dss-generator-power", type=Path, required=True)
    parser.add_argument("--interpss-generator-power", type=Path, required=True)
    ...
    validate_control_tags(args.dss_generator_power, args.interpss_generator_power,
                          args.require_control_tag)
    dss = read_generator_powers(args.dss_generator_power, args.zero_threshold_kw)
    interpss = read_generator_powers(args.interpss_generator_power, args.zero_threshold_kw)

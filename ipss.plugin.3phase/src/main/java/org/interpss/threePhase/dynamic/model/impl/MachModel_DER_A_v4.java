...
/*
 * 
 * The below model implements DER_A model
 * The internal voltage step is bypassed, as the injected current is
 * given to the network, so coverting Ip and Iq to an equivalent voltage source
 * and then back to current injection is trivial
 * 
 */

public class MachModel_DER_A_v4 extends DynGenModel3Phase{
		...
		public MachModel_DER_A_v4(){
			states = new Hashtable<>();
		}
		
		public MachModel_DER_A_v4(DStab3PGen gen) {
			this.parentGen = gen;
		...
		public boolean initStates(BaseDStabBus abus){
			 ...
			 this.Ipmin = 0;
			 this.Iqmin = -1* this.Pref / this.v_meas;
			 this.Iqmax = this.Pref / this.v_meas;
		...
		private boolean calcIpIq(int flag) {
...
//				System.out.println("voltage mag: " + this.getParentGen().getParentBus().getVoltageMag());
//			}
//			//this.nortonCurSource = new Complex(Ir, Ix);
////			
		...
		// --------------------- Output for state monitor  ----------------------------------------
		 ...
		 public Hashtable<String, Object> getStates(Object ref) {
			 ...
			 if (this.isDebugMode) {
				 ...
				 System.out.println("            Iqcmd: " + this.Iqcmd);
				 System.out.println("            Ipcmd: " + this.Ipcmd);
				 System.out.println("            NortCur: " + this.nortonCurSource);
